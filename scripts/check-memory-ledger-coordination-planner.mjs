...
const { verifiedLedger } = buildMemoryAwareCoordinationLedger({
  prefix: 'memory-ledger-coordination-planner'
});
const planner = new ProcessBackedAgentPlanner({
  ...
  plannerId: 'memory-ledger-coordination-planner-runtime'
});
...
const receipt = agent.run({
  ...
  context: {
    taskId: 'memory-ledger-coordination-planner-next-task',
    description: 'Find a graph path'
  },
  reproduction: 'memory-ledger-coordination-planner'
});

assert.equal(isTrustedMemoryAwareAgentRunReport(receipt), true);
assert.equal(receipt.memoryContext.query.source, MEMORY_SOURCES.COORDINATION);
...
assert.equal(Object.hasOwn(receipt, 'peerMessages'), false);
assert.equal(Object.hasOwn(receipt, 'rounds'), false);

console.log(
  `FLUID_MEMORY_LEDGER_COORDINATION_PLANNER_OK source=${receipt.memoryContext.query.source} `
