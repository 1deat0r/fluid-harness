...
const { ledger, report } = buildMemoryAwareAdversarialEnsemble({
  prefix: 'memory-ledger-memory-aware-ensemble-planner'
});
...
const planner = new ProcessBackedAgentPlanner({
  ...
  plannerId: 'memory-ledger-memory-aware-ensemble-planner-runtime'
});
...
const receipt = agent.run({
  ...
  context: {
    taskId: 'memory-ledger-memory-aware-ensemble-next-task',
    description: 'Find a graph path'
  },
  reproduction: 'memory-ledger-memory-aware-ensemble-planner'
});

assert.equal(receipt.memoryContext.query.source, MEMORY_SOURCES.ENSEMBLE);
assert.equal(receipt.memoryContext.query.strategyKey, 'memory-aware-agent-ensemble');
...
console.log(
  `FLUID_MEMORY_LEDGER_MEMORY_AWARE_ENSEMBLE_PLANNER_OK `
