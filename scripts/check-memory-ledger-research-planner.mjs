...
import { buildCompletedResearchMemoryLedger } from './fixtures/research-memory-ledger.mjs';

const fixturePath = fileURLToPath(new URL('./fixtures/process-boundary-candidate.mjs', import.meta.url));
const { verifiedLedger } = buildCompletedResearchMemoryLedger({
  prefix: 'memory-ledger-research-planner'
});
const planner = new ProcessBackedAgentPlanner({
  ...
  plannerId: 'memory-ledger-research-planner-runtime'
});
...
const receipt = agent.run({
  ...
  context: {
    taskId: 'memory-ledger-research-next-task',
    description: 'Find a graph path'
  },
  reproduction: 'memory-ledger-research-planner'
});

assert.equal(receipt.memoryContext.query.source, MEMORY_SOURCES.RESEARCH);
...
console.log(
  `FLUID_MEMORY_LEDGER_RESEARCH_PLANNER_OK source=${receipt.memoryContext.query.source} `
