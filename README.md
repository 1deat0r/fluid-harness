...
- Acceptance ledger: 560/560 gates marked met in [GATES.md](GATES.md).
...
In plain terms, Fluid-Harness is the control, evidence, and evaluation layer around future agents—not the agent brain and not an ASI system by itself. It is also becoming a bounded harness architect/researcher: it can help increasingly capable agents choose strategies, act, verify results, learn from evidence, retrieve bounded structured memories from prior runs, architecture experiments, coordination transcripts, supervised sessions, and pending research questions, prioritize bounded research handoffs, research alternatives, propose registered-component architecture configurations from summary-only research, adopt reproducible improvements, invoke narrowly bounded process-isolated tools, route natural-language work through an opt-in process-isolated model provider, and preserve a complete bounded run as data-only history. It now supports narrow, integrity-checked restore of learning context, agent-run history, standalone memory-aware ensemble quorum summaries, coordination transcripts, complete supervised-session summaries, pending-research summaries, and detailed architecture-discovery evidence across processes; ledger-derived memory entries also retain a frozen, read-only `{kind, sequence, hash}` locator back to their verified archive record; finite independent skeptic lineages can attack adversarial cases, aggregate weaknesses, archive the result, and inform a fresh planner or bounded proof-quorum ensemble without entering production promotion; a verifier-correlation audit can distinguish fresh runtime independence from shared verifier identifiers, preserving unresolved coverage as a review signal; a finite distribution-shift runner can test a fixed task under changed adversarial inputs with fresh dependencies per case, archive the robustness result, leave a bounded historical reminder, and feed that warning to a fresh process-isolated planner or architecture proposer without restoring execution authority; and a Harness Factory façade can run the bounded research-to-proposal-to-build-to-evaluate-to-replay-to-adopt pipeline, archive the detailed discovery transcript and explicit holdout-validation outcomes, retire evaluated candidates, build a fresh bounded agent on request, use verified archive summaries to inform the next factory generation, preserve failed strict-improvement, regression, or benchmark-drift attempts as separate observed-only evidence, and run a proposal-only `factory.proposeArchitectures()` pass that labels the same canonical configurations `NOVEL` or `REPEATED` before evaluation, optionally archives those untested batches as a separate hash-chained history, and exposes them as observed-only memory; it can also replay an explicitly archived proposal batch through fresh evaluation, rank untested archived proposal batches as a dedicated research-agenda target with a plan and hash-chained execution receipt, and account for what each such replay actually yielded as adoption, unseen-holdout, attribution, comparator, and downstream-gain evidence. Model training, rich semantic or durable memory, live production-model credentials, shared-memory or open-ended multi-agent coordination, broad live-I/O tool integrations, arbitrary architecture invention, and real-world sandboxing remain future scope.

`factory.selfDesignAndManufacture(options)` is the first end-to-end evidence-backed self-design entry point. It automatically builds bounded research context from the factory's verified ledger, passes only summary data plus exact archive locators to a process-isolated proposer, and lets that proposer choose among operator-registered planner candidates and bounded policy settings. The parent then independently resolves, evaluates, replays, adopts, archives, holdout-tests, and—when requested—constructs and proof-tests one fresh agent harness. The caller cannot inject a replacement research context or suppress the archive, and an empty evidence ledger fails before mutation. This is autonomous choice over a trusted finite design space, not arbitrary code invention, authority transfer, or deployment.

The proposal-only `factory.proposeArchitectures()` pass can optionally derive its research context from selected verified factory memory sources, including architecture discoveries, rejected improvements, architecture-coverage summaries, and archived proposal batches. The query is explicit, bounded, and historical-only; it helps a fresh proposer see what has already been tried without restoring an agent, candidate, code, proof, or authority. `factory.archiveArchitectureProposals(report)` is the explicit persistence step; `factory.architectureProposalHistory()` returns a capped read-only view, and `factory.proposeArchitectures({ archive: true })` is the convenience form. Archived proposals remain untested, unevaluated, unadopted, undeployed data; their exact fingerprints can prevent a later pass from calling the same configuration novel.

Archived proposal replay is now an explicit supervised factory bridge: `factory.manufactureFromArchivedProposals(report, options)` accepts only an exact current archived report from that factory, rebuilds proposal objects from verified summaries, reruns fresh production/research/skeptic evaluation plus independent replay and adoption, and records a new generation with the source proposal locator. The source archive remains untested, data-only, and non-authoritative; forged, pending, foreign, stale, tampered, unknown, or accessor-bearing inputs fail before ledger mutation.

Whether that archived exploration ever becomes tested design is now measurable: `factory.architectureProposalConversion()` derives a bounded, hash-verified funnel over one factory's verified archives. It reports archived batches and proposals, distinct archived architecture fingerprints, how many of those fingerprints a *later* generation or rejected-improvement attempt actually evaluated, the resulting conversion rate, and per-batch `UNTESTED`, `CONVERTED`, `REPLAYED`, or `EXHAUSTED` labels. `REPLAYED` means a generation cites that exact batch archive; `EXHAUSTED` means three fresh replays still left architecture fingerprints untested. The view is read-only and advisory: it cannot mark a proposal batch evaluated or adopted, cannot restore or promote a candidate, cannot substitute for a measured strict gain, and cannot be fed back into the replay bridge as if it were an archive.

That untested exploration can now return through the factory's own research backlog. `factory.researchAgenda()` adds a `REPLAY_ARCHIVED_PROPOSALS` target for each archived proposal batch that has never been replayed and still holds architecture fingerprints no later generation or rejected-improvement attempt evaluated. The target ranks below recovery, validation, skeptic, transfer, and strict-improvement work, so it never displaces an open obligation; `factory.researchPlan()` projects it into a frozen data-only plan naming the `ARCHIVED_PROPOSAL_REPLAY` bridge and `factory.executeArchivedProposalReplayResearch(target, options)`, and `factory.executeResearchPlanReceipt(plan, options)` can dispatch that exact current plan with a caller-supplied archived proposal report plus fresh planner candidates, cases, and budgets. The dispatch reruns the full manufacture lifecycle, archives the new generation with the source batch locator, records a hash-chained execution receipt, and clears the agenda target only from that fresh evidence. The agenda item, plan, and receipt are advisory: they carry no candidate, planner, runner, report, adoption, or deployment authority, and a forged, pending, foreign, stale, mismatched, or accessor-bearing batch fails before the ledger changes.

What a replay actually produced is now measurable too. `factory.architectureProposalReplayOutcomes()` rebuilds, from verified archives only, the yield of every archived proposal batch of this factory: whether it was replayed, whether the replayed generation earned adoption, whether it passed an unseen holdout, whether the evaluated winner was one of the batch's own archived architecture fingerprints, how it compares against the immediately preceding archived generation on the same benchmark contract (`GAINED`, `UNCHANGED`, `REGRESSED`, `NO_COMPARATOR`, or `COMPARATOR_MISMATCH`), and how many later strict-improvement attempts — accepted or rejected — built on that generation. Rates are derived from those counts, the returned list is capped and frozen, and the same summary is available as the observed-only `HARNESS_FACTORY_ARCHITECTURE_PROPOSAL_REPLAY_OUTCOME` memory source. The comparator result is explicitly derived evidence: the replay bridge never runs the strict-gain guard, so this view can neither mint an archived gain, mark a batch evaluated, restore a candidate, nor substitute for fresh adoption or unseen-holdout validation.

Retries no longer collapse into that batch's latest outcome. `factory.architectureProposalReplayAttemptOutcomes()` returns every archived replay attempt in generation order with an exact per-batch attempt ordinal, its own adoption, attribution, holdout, immediate-predecessor comparison, and downstream-improvement counts. The report aggregates rates by attempt, caps the returned chronological tail at 24 entries, round-trips from the verified ledger, and leaves `architectureProposalReplayOutcomes()` as the compatible latest-outcome-per-batch view. Replay-outcome memory now also records the exact total attempt count. Both views are frozen historical evidence only: neither can recreate a proposal, candidate, runner, report, proof, or replay authority.

Retrying unfinished exploration is now tracked instead of forgotten. `factory.architectureProposalConversion()` counts every architecture a later generation actually evaluated, so a design that was tested and lost is no longer reported as untested, and each batch records how many archived generations cited it (`replayCount`). `factory.researchAgenda()` keeps a batch whose replay adopted nothing in the backlog and publishes its `replayAttemptCount`; scheduling chooses the fewest-attempted batch first and uses archive order as the deterministic FIFO tie-breaker. A capped agenda therefore gives every untouched batch a turn before retrying a failed one, then rotates equally attempted work without changing stable backlog identities. Each batch has a hard three-attempt limit. After the third failed replay, its conversion status becomes `EXHAUSTED`, its final receipt reports `targetResolved: true`, it leaves the agenda, and any fourth direct or planned replay fails before ledger mutation. The observed-only conversion memory records the exhaustion count and limit for a fresh proposer. Exhaustion grants no credit: the archived source remains unevaluated, unadopted, undeployed, and data-only, and only actual fresh evaluation evidence can change its conversion measurements.
...
node scripts/check-harness-factory-self-design.mjs
node scripts/check-harness-factory-self-design-agent.mjs
node scripts/check-harness-factory-self-design-evidence-boundary.mjs
node scripts/check-harness-factory-self-design-provenance.mjs
node scripts/check-harness-factory-self-design-authority-boundary.mjs
...
node scripts/check-harness-factory-benchmark-frontier-validation-research-agenda.mjs
...
The `HarnessFactory` façade is the first lifecycle-level assembly line over those authorities. It accepts an optional trusted historical research context, commissions the process-isolated proposer, resolves only registered components, evaluates and independently replays the resulting bundles, archives the detailed discovery record, protects the fresh adopted candidate, retires the evaluated candidates, and returns only a frozen summary of what happened. When an agent task is requested, it also constructs a fresh bounded runtime from the adoption, runs that task through the parent proof path, and archives only a data-only run summary; the live agent and action report never escape the factory report. Its `improve()` path round-trips the factory’s own verified ledger through bounded structured-memory sources—including discovery, benchmark, frontier, stability, research-plan execution, and proposal-history summaries—and gives the next proposer only historical data, so every new generation must earn fresh evaluation and adoption again. Each factory archive now carries a generation number, predecessor hash locator, and fingerprint of the benchmark cases/tasks/context plus mode budgets. The factory compares the new winner’s measured production, research, skeptic, transfer, and weakness metrics plus the benchmark contract with the latest verified generation by default; an optional positive `baselineGeneration` selects an older verified generation from the same factory history, allowing a prior benchmark partition to be improved after a newer partition exists. The selected baseline must exist, precede the new archive, and match the current benchmark contract; the global predecessor remains the latest archive so the ledger stays one auditable chain. An adopted candidate that regresses, merely ties, or quietly changes the selected benchmark is rejected before it is archived or exposed, while a rejected attempt remains an explicit data-only outcome. Retirement means the factory will not accept that candidate for another disposal cycle, not that JavaScript can force garbage collection or invalidate an external reference. The factory still does not deploy, mutate source code, invent unregistered primitives, or turn an archive into authority.

`factory.architectureCoverage()` is a read-only exploration ledger over the verified factory records. It counts distinct architecture fingerprints, marks repeated attempts, separates adopted generations from rejected improvements, caps the returned attempt list, and exposes only archive locators plus bounded identity/status data. The aggregate can also enter structured memory as `HARNESS_FACTORY_ARCHITECTURE_COVERAGE`, giving a later proposer an observed signal about whether exploration is actually producing new configurations. It cannot restore candidates, runners, reports, code, or authority, and it does not decide that a configuration is better.

`factory.archiveArchitectureProposals(report)` adds a separate, explicit archive for proposal-only exploration. It stores finite planner-plus-policy suggestions, exact architecture fingerprints, novelty/repetition labels, and bounded research-source counts as hash-chained data-only evidence; `factory.architectureProposalHistory()` returns at most 32 batches for that factory. The `HARNESS_FACTORY_ARCHITECTURE_PROPOSAL` memory source lets a later proposer see those untested suggestions, and future novelty checks include their exact fingerprints. Proposal history never becomes evaluated, adopted, deployed, executable, or authoritative state; the default `factory.proposeArchitectures()` call remains ledger-preserving unless `archive: true` is requested. `factory.manufactureFromArchivedProposals(report, options)` is the supervised bridge back into fresh evaluation and records exact `proposalArchive` provenance on the resulting generation. `factory.architectureProposalConversion()` reads that archive against verified generations and rejected improvements to report how much archived exploration has actually been tested, with no effect on the archive itself.
...
The memory index can also combine a bounded set of trusted sources in one read-only query: `memory.query({ sources: [...] })` returns the union of matching historical entries with a normalized, frozen source list and deterministic `sourceCounts` (including zero for a selected source with no matches). `factory.improve({ memoryQuery: { sources: [...] } })` can use that combined context so a fresh proposer sees, for example, both architecture-discovery outcomes, proposal-history suggestions, and research-plan execution receipts plus the coverage split. The context remains observed historical data only; fresh evaluation, replay, strict-gain, and adoption gates still decide whether a new generation is accepted. Empty, duplicate, unknown, accessor-bearing, conflicting, or authority-bearing filters are rejected before mutation.
...
Repeated equivalent frontiers also produce one bounded `MEMORY_SOURCES.HARNESS_FACTORY_BENCHMARK_FRONTIER_VALIDATION_STABILITY` record per repeated frontier. It tells the next proposer whether the configuration is consistently `stable` or `unstable`, with campaign counts, completeness/reproducibility/independence keywords, and bounded `variable-point-<candidate>-<level>` handles when identifiers fit the memory limit; `factory.improve({ memoryQuery: { source: MEMORY_SOURCES.HARNESS_FACTORY_BENCHMARK_FRONTIER_VALIDATION_STABILITY } })` can use that historical signal without importing runtime artifacts or authority.

`factory.benchmarkFrontierValidationStability()` groups equivalent frontiers across separate archived campaigns. A frontier is `STABLE` only after at least two independent campaigns validate every point with passing, complete, reproducible, independent evidence; one campaign is `INSUFFICIENT`, while any incomplete or failed campaign makes the repeated frontier `UNSTABLE`. Its point-level scores identify which candidate/level pairs are stable or variable across those campaigns and retain per-campaign status plus validation locators, so an unstable frontier can be investigated without guessing. This is a longitudinal research signal, not a promotion or deployment decision.
...
- `src/evidence-ledger.mjs` — append-only, hash-chained action/cycle/core/agent-run/adversarial-lineage/ensemble/distribution-shift/memory-aware-ensemble/architecture-discovery/coordination/session/proposal-history and memory-aware-transcript evidence archive boundary
...
- `src/harness-factory.mjs` — lifecycle façade for research intake, architecture manufacture, archived-proposal replay, fair bounded replay backlog and receipts, replay attempt/exhaustion accounting, batch and chronological attempt outcome accounting, proposal-conversion measurement, proposal-history archive, archive inspection, and candidate retirement
- `src/harness-factory-limits.mjs` — shared finite replay-attempt policy used by factory execution and data-only memory summaries
