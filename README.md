...
- Acceptance ledger: 545/545 gates marked met in [GATES.md](GATES.md).
...
Retrying unfinished exploration is now tracked instead of forgotten. `factory.architectureProposalConversion()` counts every architecture a later generation actually evaluated, so a design that was tested and lost is no longer reported as untested, and each batch records how many archived generations cited it (`replayCount`). `factory.researchAgenda()` therefore keeps a batch whose replay adopted nothing in the backlog instead of retiring it on a failed attempt and publishes that batch's `replayAttemptCount`. Inside the replay priority group, scheduling chooses the fewest-attempted batch first and uses archive order as the deterministic FIFO tie-breaker. A capped agenda therefore gives every untouched batch a turn before retrying a failed one, then rotates equally attempted work without changing stable backlog identities or dropping unfinished exploration. `factory.executeResearchPlanReceipt` can dispatch that re-queued target again through the same `ARCHIVED_PROPOSAL_REPLAY` bridge; every attempt archives its own generation and its own receipt, and the receipt reports `targetResolved: false` until the backlog itself stops asking for that batch. Attempt and outcome counts also reach a fresh process-isolated proposer as observed-only `HARNESS_FACTORY_ARCHITECTURE_PROPOSAL_REPLAY_OUTCOME` memory. None of this grants credit: repeated replays of a design the lifecycle keeps refusing still leave the batch unevaluated, unadopted, and data-only in the archive, and only a fresh generation with real evaluation evidence can change that.
...
- `src/harness-factory.mjs` — lifecycle façade for research intake, architecture manufacture, archived-proposal replay, replay research backlog and receipts, replay attempt accounting, replay outcome accounting, proposal-conversion measurement, proposal-history archive, archive inspection, and candidate retirement
